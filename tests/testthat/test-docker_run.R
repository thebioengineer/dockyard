...
test_with_docker("`docker_run` will provide a docker_connection and name,
  ...
  expect_true(!is.null(attr(d_conn,".name")))
})

test_with_docker("`docker_run` will provide reason for failing to create the container", {
  ...
  expect_error(
    docker_run(image = "rocker/shiny",
               name = attr(d_conn,".name"),
               ports = "9876:3838")
    )
})
